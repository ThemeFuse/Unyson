...
function fw_try_make_writable($path) {
	...
	do {
		...
		// Extract relative path: '/root/dir' . $relPath
		{
			$rel_path = explode($root_dir, $path);

			if (!empty($rel_path[0]))
				break; // is not located in root dir

			array_shift($rel_path); // remove first empty element

			$rel_path = implode($root_dir, $rel_path);
		}

		$rel_path_arr = explode('/', $rel_path);
		array_shift($rel_path_arr); // remove first empty element
